   ...
   //     component.set("v.isSpinner",true);
    ...
    UpdateListPriceDiscount: function(component,Type,ItemId,PricingMethod,fullOrderList) {
     ...
     //  var UsageQuantity;
        ...
        if(PricingMethod=='ProductDiscountGroupUsageBased' || PricingMethod=='ProductUsageBased' || PricingMethod=='OrderProductQuantity'){ 
                for(var key in fullOrderList){
                    ...
                    console.log('**ProductId Two:c**'+ fullOrderList[key].ItemGroupProductId);
                        ...
                        if((ProductId ==  fullOrderList[key].ProductId && fullOrderList[key].ItemGroupComponent == false) || fullOrderList[key].ItemGroupProductId == ProductId){ // Commented out for TEST
                                var varQuantity;
                               ...
                               // ProdGroup = fullOrderList[key].ProductName+ fullOrderList[key].listPriceDiscountType+ fullOrderList[key].CustomerDiscountGroup;
       ...
       for(var key in ListPriceDiscountFull){ 
        //console.log('**LPD Update 2: CustomerDiscountGroup**'+ ListPriceDiscountFull[key].CustomerDiscountGroup__c);
        //console.log('**LPD Update 2: ProductDiscountGroup**'+ ListPriceDiscountFull[key].ProductDiscountGroup__c);
        //console.log('**LPD Update 2: Subsidiary**'+ ListPriceDiscountFull[key].Subsidiary__c);

            if(ListPriceMap[ListPriceDiscountFull[key].CustomerDiscountGroup__c] ==1 && ListPriceMap[ListPriceDiscountFull[key].ProductDiscountGroup__c] == 1 && ListPriceMap[ListPriceDiscountFull[key].Subsidiary__c] == 1){
        ...
        ItemGrpMap =[];
         for(var key in fullOrderList){
            ...
            if((ProductId ==  fullOrderList[key].ProductId && fullOrderList[key].ItemGroupComponent == false) || fullOrderList[key].ItemGroupProductId == ProductId){ 
                ...
                if(fullOrderList[key].PriceDiscount == 'Free' || fullOrderList[key].PriceDiscount =='Service'){
                   // fullOrderList[key].STDPrice = 0;
                    ...
                    fullOrderList[key].NoDiscAmount = fullOrderList[key].Quantity * fullOrderList[key].BasePrice;
                        ...
                        else if(PricingMethod=='ProductDiscountGroupUsageBased'){ // && ((curItemId == fullOrderList[key].ItemId || curItemId == fullOrderList[key].ItemGroup ) || MultiShiptoSelect > 1)){ 
                           
                    
                          //  ProdGroup = fullOrderList[key].ProductRateGroup;
                          //  ProdGroup = fullOrderList[key].ProductName+ fullOrderList[key].listPriceDiscountType+ fullOrderList[key].CustomerDiscountGroup;
                        ...
                        else if(PricingMethod=='ProductUsageBased'){ // && ((curItemId == fullOrderList[key].ItemId || curItemId == fullOrderList[key].ItemGroup ) || MultiShiptoSelect > 1) ){ 
                           
                            ProdGroup = fullOrderList[key].ProductName;

                           // ProdGroup = PricingMethod;
                         //   ProdGroup = fullOrderList[key].ProductName+ fullOrderList[key].listPriceDiscountType+ fullOrderList[key].CustomerDiscountGroup; 
                         /*
                            if(OrderProdMap[PricingMethod] != NaN && OrderProdMap[PricingMethod] != undefined && OrderProdMap[PricingMethod] != null){
                                OrderProdMap[fullOrderList[key].ItemId] =parseFloat( OrderProdMap[ProdGroup]) + parseFloat( OrderProdMap[PricingMethod]);
                            }
                            else{
                                OrderProdMap[fullOrderList[key].ItemId] =parseFloat( OrderProdMap[ProdGroup]);     
                            }
                       ...
                       console.log('**Update 1: CustomerDiscountGroup:' + fullOrderList[key].CustomerDiscountGroup);
                        ...
                        for(var keyLP in ListPriceDiscount){     
                            ...
                            console.log('**Update 2: Subsidiary** '+ListPriceDiscount[keyLP].Subsidiary__c);
                                ...
                                if((ListPriceDiscount[keyLP].CustomerDiscountGroup__c == fullOrderList[key].CustomerDiscountGroup) 
                                    && OrderProdMap[ProdGroup] >= ListPriceDiscount[keyLP].StartQuantity__c && listPriceSelected==false && ListPriceDiscount[keyLP].Subsidiary__c == fullOrderList[key].Subsidiary){
                             
                                    console.log('**Component Update: Before 3**');
                                 if(ListPriceDiscount[keyLP].ProductDiscountGroup__c == fullOrderList[key].listPriceDiscountType){ 
                                          loopCountPRODDTYEE++;
                                            console.log('**Component Update: Before 3**');
                                            if((ListPriceDiscount[keyLP].OrderLineType__c == fullOrderList[key].PriceDiscount) && listPriceSelected==false){ // PriceDiscount comes from listPriceProductType ---  &&  (ListPriceDiscount[keyLP].OrderLineType__c == fullOrderList[key].listPriceProductType[type] )
                                            ...
                                            if(ListPriceDiscount[keyLP].DiscountType__c!='Market Segment' || (ListPriceDiscount[keyLP].DiscountType__c=='Market Segment' && ListPriceDiscount[keyLP].MarketSegment__c == fullOrderList[key].MarketSegment)){
                                                            ...
                                                            console.log('**Component Update: In 1**'+fullOrderList[key].ComponentQuantityFinal);
        ...
        for(var key in fullOrderList){
            ...
            if(ProductId ==  fullOrderList[key].ProductId && fullOrderList[key].ItemGroupComponent == true){
                ...
                console.log('**Item group Update: In**');
